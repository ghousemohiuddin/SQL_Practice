...
-- MySQL doesnot support full outer join instead we can use union of right and left joins to get full join
SELECT e.emp_name, d.dept_name
FROM employee e 
LEFT JOIN department d ON e.drpt_id=d.dept_id
UNION
SELECT e.emp_name, d.dept_name
FROM employee e 
RIGHT JOIN department d ON e.drpt_id=d.dept_id;
...
WHERE e.salary > 15000
AND  e.dept_id = d.dept_id ; 
-- This is unsupported in PostgreSQL but works in Oracle. Other RDBMS may have alternative symbols to performs same thing.
